...
async function refresh(){try{var sr=await fetch('/api/status'),sd=await sr.json();var tr=await fetch('/api/topology'),td=await tr.json();document.getElementById('loading').style.display='none';document.getElementById('content').style.display='block';document.getElementById('totalPeers').textContent=sd.total_peers;document.getElementById('onlinePeers').textContent=sd.online;document.getElementById('offlinePeers').textContent=sd.offline;document.getElementById('uptime').textContent=fmtU(sd.uptime);var rc=document.getElementById('ruleCount');if(td.rules){var parts=td.rules.split('|');rc.textContent=parts.length+' rules'}var tb=document.getElementById('peerTable');tb.innerHTML='';for(var p of sd.peers){var dot=p.online?'online':'offline',lbl=p.online?ago(p.last_hello):ago(p.last_hello)+' (off)';var tr=document.createElement('tr');tr.innerHTML='<td><span class="status-dot '+dot+'"></span>'+(p.online?'Online':'Offline')+'</td><td><strong>'+p.name+'</strong></td><td class="mono" style="font-size:.75em;color:#8b949e">'+p.uuid.substring(0,8)+'</td><td class="mono">'+p.internal_ip+'</td><td class="mono" style="font-size:.85em">'+(p.endpoint||'N/A')+'</td><td class="ago">'+lbl+'</td>';tb.appendChild(tr)}drawGraph(td,document.getElementById('graphSvg'))}catch(e){console.error('refresh failed',e);document.getElementById('loading').textContent='Connection lost. Retrying...'}}
